  ...
  function renderGateList(force) {
    ...
    listEl.innerHTML = rows.map(function (sp) {
      ...
      var switchHtml = sp.status === 'open'
        ? '<div class="cp-gate-card__switch' + (isHomeDisplay ? ' is-on' : '') + '" data-cp-home-switch="' + esc(sp.contactId) + '" role="switch" aria-checked="' + (isHomeDisplay ? 'true' : 'false') + '" title="' + (isHomeDisplay ? '取消主页显示' : '显示到主页') + '" onclick="var e=window.event||arguments[0];if(e){e.stopPropagation();e.preventDefault();}var cid=this.getAttribute(\'data-cp-home-switch\');var st=window.miyaCoupleStore;if(st&&cid){var cur=st.getHomeDisplayContactId?st.getHomeDisplayContactId():\'\';var spaces=st.getSpaces?st.getSpaces():[];var openCount=spaces.filter(function(s){return s&&s.status===\'open\';}).length;if(cur===cid){if(openCount<=1){if(window.miyaCoupleApp&&window.miyaCoupleApp.toast){window.miyaCoupleApp.toast(\'至少要保留一個人設\');}}else{st.setHomeDisplayContactId(\'\');}}else{st.setHomeDisplayContactId(cid);}if(window.miyaCoupleApp&&window.miyaCoupleApp.renderAll){window.miyaCoupleApp.renderAll();}if(window.miyaRefreshCustomLayout){window.miyaRefreshCustomLayout();}}return false;">' +
